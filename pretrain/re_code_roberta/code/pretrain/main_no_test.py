...
if __name__ == "__main__":
    ...
    if args.model == "MTB":
        ...
        if args.debug == 0:
            if not os.path.isfile('train_dataset_CP_R_mask_alpha_no_sep'):
                train_dataset = CP_R_Dataset("../../data/CP_R", args)
                torch.save(train_dataset, 'train_dataset_CP_R_mask_alpha_no_sep')
            else:
                train_dataset = torch.load('train_dataset_CP_R_mask_alpha_no_sep')
        elif args.debug == 1:
