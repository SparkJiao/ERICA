...
if __name__ == "__main__":
    ...
    if args.model == "MTB":
        ...
        print('preparing data')
        if args.debug == 0:
            if not os.path.isfile('train_dataset_CP_R_mask'):
                #if True:
                train_dataset = CP_R_Dataset("../../data/CP_R", args)
                torch.save(train_dataset, 'train_dataset_CP_R_mask')
            else:
                train_dataset = torch.load('train_dataset_CP_R_mask')
        elif args.debug == 1:
