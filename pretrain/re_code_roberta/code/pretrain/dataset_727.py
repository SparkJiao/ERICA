...
class CP_R_Dataset(data.Dataset):
    ...
    def __init__(self, path, args):
        ...
        entityMarker = EntityMarker()
        self.idx2token = {v: k for k,v in entityMarker.tokenizer.vocab.items()}
        ...
        '''
        for k in random.sample(list(self.ent2sen.keys()), 100):
            sentence = self.ent2sen[k][0]
            h_p = sentence["h"]["pos"][0]
            t_p = sentence["t"]["pos"][0]
            print(sentence['tokens'])
            print(r2n[sentence['r']])
            print(sentence['tokens'][h_p[0]: h_p[-1]+1])
            print(sentence['tokens'][t_p[0]: t_p[-1]+1])
            input()
        '''
        ...
        for k in self.ent2sen:
            ...
            for sentence in self.ent2sen[k]:
                ...
                #ids, ph, pt, ph_l, pt_l = entityMarker.tokenize(sentence["tokens"], [h_p[0], h_p[-1]+1], [t_p[0], t_p[-1]+1], None, None, h_flag, False, 1)
                ids, ph, pt, ph_l, pt_l = entityMarker.tokenize(sentence["tokens"], [h_p[0], h_p[-1]+1], [t_p[0], t_p[-1]+1], None, None, h_flag, True, 1)

                bag.append([ids, ph, pt, ph_l, pt_l, sentence['r']])

                #ids, ph, pt, ph_l, pt_l = entityMarker.tokenize(sentence["tokens"], [h_p[0], h_p[-1]+1], [t_p[0], t_p[-1]+1], None, None, False, False, 3)
                ids, ph, pt, ph_l, pt_l = entityMarker.tokenize(sentence["tokens"], [h_p[0], h_p[-1]+1], [t_p[0], t_p[-1]+1], None, None, True, True, 3)
                ids = ids[1: ]
                ph -= 1
                pt -= 1
                ph_l -= 1
                ...
                #ids, ph, pt, ph_l, pt_l = entityMarker.tokenize(sentence["tokens"], [h_p[0], h_p[-1]+1], [t_p[0], t_p[-1]+1], None, None, False, t_flag, 2)
                ids, ph, pt, ph_l, pt_l = entityMarker.tokenize(sentence["tokens"], [h_p[0], h_p[-1]+1], [t_p[0], t_p[-1]+1], None, None, True, t_flag, 2)
                ids = ids[1: ]
                ph -= 1
                pt -= 1
                ph_l -= 1
