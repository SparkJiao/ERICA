...
def log_loss(step_record, loss_record):
    if not os.path.exists("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/res"):
        os.mkdir("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/res")
    ...
    plt.grid(True)
    plt.savefig(os.path.join("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/res", 'loss_curve.png'))
    plt.close()
    ...
    for i in range(args.max_epoch):
        for step, batch in enumerate(train_dataloader):
            if args.model == "MTB":
                ...
                batch_2 = batch[1]
                batch_3 = batch[2]
                input_R = torch.cat([batch_2[0].to(args.device)[: 8], batch_3[0].to(args.device)[8: ]], dim = 0)
                mask_R = torch.cat([batch_2[1].to(args.device)[: 8], batch_3[1].to(args.device)[8: ]], dim = 0)
                label_R = torch.cat([batch_2[2].to(args.device)[: 8], batch_3[2].to(args.device)[8: ]], dim = 0)
                h_pos_R = torch.cat([batch_2[3].to(args.device)[: 8], batch_3[3].to(args.device)[8: ]], dim = 0)
                t_pos_R = torch.cat([batch_2[4].to(args.device)[: 8], batch_3[4].to(args.device)[8: ]], dim = 0)
                ...
                "input_R": input_R, "mask_R": mask_R, "label_R": label_R, "h_pos_R": h_pos_R, 't_pos_R': t_pos_R}
            model.train()
            if args.model == "CP_R":
                m_loss, m_loss_R, r_loss, r_loss_R = model(**inputs)

                loss = (m_loss + m_loss_R + r_loss + r_loss_R)/2
            else:
            ...
            if step % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [0, -1] and global_step % args.save_step == 0:
                    if not os.path.exists("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/ckpt"):
                        os.mkdir("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/ckpt")
                    if not os.path.exists("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/ckpt/"+args.save_dir):
                        os.mkdir("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/ckpt/"+args.save_dir)
                    ckpt = {
                        'bert-base': model.module.model.bert.state_dict(),
                    }
                    torch.save(ckpt, os.path.join("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/ckpt/"+args.save_dir, "ckpt_of_step_"+str(global_step)))
                ...
                if args.local_rank in [0, -1]:
                    if args.model == "CP_R":
                        sys.stdout.write("step: %d, shcedule: %.3f, mlm_loss: %.6f mlm_loss_R: %.6f relation_loss: %.6f relation_loss_R: %.6f\r" % (global_step, global_step/step_tot, m_loss, m_loss_R, r_loss, r_loss_R))
                    else:
...
if __name__ == "__main__":
    ...
    # log train
    if args.local_rank in [0, -1]:
        if not os.path.exists("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/log"):
            os.mkdir("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/log")
        with open("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/log/pretrain_log", 'a+') as f:
    ...
    if args.model == "MTB":
        ...
        if not os.path.isfile('train_dataset_MTB'):
            train_dataset = MTBDataset("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/data/MTB", args)
        ...
        if not os.path.isfile('train_dataset_CP'):
            train_dataset = CPDataset("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/data/CP", args)
        ...
        if not os.path.isfile('train_dataset_CP_R'):
            #if True:
            train_dataset = CP_R_Dataset("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/data/CP_R", args)
        ...
        if not os.path.isfile('train_dataset_CP_large'):
            train_dataset = CPDataset("/mnt/yardcephfs/mmyard/g_wxg_td_prc/yujiaqin/REAnalysis/data/CP_R_large", args)
