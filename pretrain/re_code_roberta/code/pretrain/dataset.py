...
class CPDataset(data.Dataset):
    ...
    def __init__(self, path, args):
        ...
        self.args = args 
        if args.debug == 0:
            data = json.load(open(os.path.join(path, "cpdata_convert.json")))
            rel2scope = json.load(open(os.path.join(path, "rel2scope_convert.json")))
        ...
        for i, sentence in enumerate(data):
            ...
            ids, ph, pt, ph_l, pt_l = entityMarker.tokenize(sentence["tokens"], [h_p[0], h_p[-1]+1], [t_p[0], t_p[-1]+1], None, None, h_flag, t_flag, 1)
...
class CP_R_Dataset(data.Dataset):
    ...
    def __init__(self, path, args):
        ...
        #         e1 = self.get_sen(random.choice(ent2sen_tok[item_1[0]]), h_flag, hop_flag, 1, 1)
        #         e2 = self.get_sen(random.choice(ent2sen_tok[item_1[1]]), hop_flag, t_flag, 3, 3)
        #         all_pos_pair.append([e1, e2])
        #     return all_pos_pair

        def get_pos_test(relation2sen_soft_t, ent2sen_tok):
            all_pos_pair = []
            ...
            for i, item_1 in enumerate(relation2sen_soft_t):
                ...
                sample_list_e1 = random.sample(ent2sen_tok[item_1[0]], min(len(ent2sen_tok[item_1[0]]), 2))
                sample_list_e2 = random.sample(ent2sen_tok[item_1[1]], min(len(ent2sen_tok[item_1[1]]), 2))

                if len(sample_list_e1) == 2 and len(sample_list_e2) == 2:
                    pos_list = [1,2,2,3]
                    sent_idx = [0,1,2,3]
                    random.shuffle(sent_idx)
                    e1 = self.get_sen(sample_list_e1[0], h_flag, hop_flag, 1, pos_list[sent_idx[0]])
                    e2 = self.get_sen(sample_list_e2[0], hop_flag, t_flag, 3, pos_list[sent_idx[1]])
                    e1_2 = self.get_sen(sample_list_e1[1], h_flag, hop_flag, 1, pos_list[sent_idx[2]])
                    e2_2 = self.get_sen(sample_list_e2[1], hop_flag, t_flag, 3, pos_list[sent_idx[3]])
                elif len(sample_list_e1) == 2 and len(sample_list_e2) == 1:
                    pos_list = [1,2,3]
                    sent_idx = [0,1,2]
                    random.shuffle(sent_idx)
                    e1 = self.get_sen(sample_list_e1[0], h_flag, hop_flag, 1, pos_list[sent_idx[0]])
                    ...
                    e1_2 = self.get_sen(sample_list_e1[1], h_flag, hop_flag, 1, pos_list[sent_idx[2]])
                    e2_2 = None
                    sent_idx.insert(3, None)
                elif len(sample_list_e1) == 1 and len(sample_list_e2) == 2:
                    pos_list = [1,2,3]
                    sent_idx = [0,1,2]
                    random.shuffle(sent_idx)
                    e1 = self.get_sen(sample_list_e1[0], h_flag, hop_flag, 1, pos_list[sent_idx[0]])
                    e2 = self.get_sen(sample_list_e2[0], hop_flag, t_flag, 3, pos_list[sent_idx[1]])
                    e1_2 = None
                    e2_2 = self.get_sen(sample_list_e2[1], hop_flag, t_flag, 3, pos_list[sent_idx[2]])
                    sent_idx.insert(2, None)
                else:
                    pos_list = [1,3]
                    sent_idx = [0,1]
                    random.shuffle(sent_idx)
                    ...
                    sent_idx.insert(2, None)
                    sent_idx.insert(3, None)
                all_pos_pair.append([e1, e2, e1_2, e2_2, sent_idx])
            return all_pos_pair
        ...
        for rel in self.relation_map_t:
            ...
            pos_R = get_pos_test(relation2sen_soft_t_test, self.ent2sen_tok)
    ...
    def get_test_set(self):
        ...
        for i, idx in enumerate(neg_test):
            ...
            h_pos[i], t_pos[i], h_pos_l[i], t_pos_l[i] = self.h_pos[idx], self.t_pos[idx], self.h_pos_l[idx], self.t_pos_l[idx]
            ...
            # print([r2n[id2rel[self.label[idx]]]])
            # s_y = [self.idx2token[x] for x in self.tokens[idx] if x != 0]
            # print(' '.join(s_y))
            # print(s_y[h_pos[i]: h_pos_l[i]])
            # print(s_y[t_pos[i]: t_pos_l[i]])
            # print('\n')
        ...
        input_doc = np.zeros((len(pos_R_test_t), self.args.max_length * 4), dtype=int)
        mask_doc = np.zeros((len(pos_R_test_t), self.args.max_length * 4), dtype=int)
        label_doc = np.zeros((len(pos_R_test_t)), dtype=int)
        h_doc = np.zeros((len(pos_R_test_t), 2), dtype=int)
        t_doc = np.zeros((len(pos_R_test_t), 2), dtype=int)
        h_l_doc = np.zeros((len(pos_R_test_t), 2), dtype=int)
        t_l_doc = np.zeros((len(pos_R_test_t), 2), dtype=int)

        for test_idx, ind in enumerate(pos_R_test_t):
            ids_1, ph_1, pt_1, ph_l_1, pt_l_1, r_1 = ind[0]
            ids_2, ph_2, pt_2, ph_l_2, pt_l_2, r_2 = ind[1]
            if ind[2] != None:
                ids_1_d, ph_1_d, pt_1_d, ph_l_1_d, pt_l_1_d, r_1_d = ind[2]
            else:
                ids_1_d, ph_1_d, pt_1_d, ph_l_1_d, pt_l_1_d, r_1_d = [], 0, 0, 0, 0, -1
            if ind[3] != None:
                ids_2_d, ph_2_d, pt_2_d, ph_l_2_d, pt_l_2_d, r_2_d = ind[3]
            else:
                ids_2_d, ph_2_d, pt_2_d, ph_l_2_d, pt_l_2_d, r_2_d = [], 0, 0, 0, 0, -1
            sent_idx = ind[4]

            ori_ids = [ids_1, ids_2, ids_1_d, ids_2_d]
            ids_list = [None, None, None, None]
            for ids_idx in range(4):
                if sent_idx[ids_idx] != None:
                    ids_list[sent_idx[ids_idx]] = ori_ids[ids_idx]
            for i in range(4):
                if ids_list[i] == None:
                    ids_list[i] = []
            ids = ids_list[0] + ids_list[1] + ids_list[2] + ids_list[3]
            ids_list_len = [0, len(ids_list[0]), len(ids_list[0] + ids_list[1]), len(ids_list[0] + ids_list[1] + ids_list[2])]

            if ind[2] != None:
                ph_r3 = [ids_list_len[sent_idx[0]] + ph_1, ids_list_len[sent_idx[2]] + ph_1_d]
                ph_l_r3 = [ids_list_len[sent_idx[0]] + ph_l_1, ids_list_len[sent_idx[2]] + ph_l_1_d]
            else:
                ph_r3 = [ids_list_len[sent_idx[0]] + ph_1, ids_list_len[sent_idx[0]] + ph_1]
                ph_l_r3 = [ids_list_len[sent_idx[0]] + ph_l_1, ids_list_len[sent_idx[0]] + ph_l_1]
            if ind[3] != None:
                pt_r3 = [ids_list_len[sent_idx[1]] + pt_2, ids_list_len[sent_idx[3]] + pt_2_d]
                pt_l_r3 = [ids_list_len[sent_idx[1]] + pt_l_2, ids_list_len[sent_idx[3]] + pt_l_2_d]
            else:
                pt_r3 = [ids_list_len[sent_idx[1]] + pt_2, ids_list_len[sent_idx[1]] + pt_2]
                pt_l_r3 = [ids_list_len[sent_idx[1]] + pt_l_2, ids_list_len[sent_idx[1]] + pt_l_2]

            ph_r3 = [min(self.args.max_length*4-1, x) for x in ph_r3]
            pt_r3 = [min(self.args.max_length*4-1, x) for x in pt_r3]
            ph_l_r3 = [min(self.args.max_length*4, x) for x in ph_l_r3]
            pt_l_r3 = [min(self.args.max_length*4, x) for x in pt_l_r3]

            # r2n = json.load(open('/data2/private/qinyujia/data_gen/zero_shot_data/relation_name_desc.json', 'r'))
            # r2n = {k: v['name'] for k,v in r2n.items()}
            # id2rel = {v:k for k,v in self.rel2id.items()}
            # try:
            #     print([r2n[r_1], r2n[r_2], r2n[self.relation_map_t['#'.join([r_1, r_2])]]])
            #     s_y = [self.idx2token[x] for x in ids if x != 0]
            #     print(' '.join(s_y))
            #     for i in range(2):
            #         print(s_y[ph_r3[i]: ph_l_r3[i]])
            #         print(s_y[pt_r3[i]: pt_l_r3[i]])
            #     print('\n')
            # except:
            #     print('wrong here')

            length = min(len(ids), self.args.max_length * 4)

            input_doc[test_idx, :length] = ids[:length]
            mask_doc[test_idx, :length] = 1

            label_doc[test_idx] = np.array([self.rel2id[self.relation_map_t['#'.join([r_1, r_2])]]], dtype=int)
            h_doc[test_idx], t_doc[test_idx], h_l_doc[test_idx], t_l_doc[test_idx] = np.array(ph_r3, dtype=int), np.array(pt_r3, dtype=int), np.array(ph_l_r3, dtype=int), np.array(pt_l_r3, dtype=int)

        for x in [input_doc, mask_doc, label_doc, h_doc, t_doc, h_l_doc, t_l_doc]:
            x = torch.from_numpy(x)
        input_doc = torch.from_numpy(input_doc)
        mask_doc = torch.from_numpy(mask_doc)
        label_doc = torch.from_numpy(label_doc)
        h_doc = torch.from_numpy(h_doc)
        t_doc = torch.from_numpy(t_doc)
        h_l_doc = torch.from_numpy(h_l_doc)
        t_l_doc = torch.from_numpy(t_l_doc)
        ...
        two_hop_pos = (input_doc.to(self.args.device), mask_doc.to(self.args.device), label_doc, h_doc.to(self.args.device), t_doc.to(self.args.device), h_l_doc.to(self.args.device), t_l_doc.to(self.args.device))
        return single_sentence, two_hop_pos

    def get_test_set_old(self):
        pos_R_test_t, neg_test = self.pos_R_test_t, self.neg_test
        #prepare neg_test
        input = np.zeros((len(neg_test), self.args.max_length), dtype=int)
        mask = np.zeros((len(neg_test), self.args.max_length), dtype=int)
    ...
    def get_sen(self, e, h_flag, t_flag, sen_type = 1, pos_type = 1):
    ...
    def __pos_pair_doc__(self, scope_r1, scope_r2, scope_r3, relation2sen_soft_t, ent2sen_tok, max_sample_num):
        ...
        for i, item_1 in enumerate(relation2sen_soft_t):
            ...
            e_single_r1 = random.choice(pos_scope_r1)
            while e_single_r1 in self.ignore_id:
                e_single_r1 = random.choice(pos_scope_r1)
            
            e_single_r2 = random.choice(pos_scope_r2)
            while e_single_r2 in self.ignore_id:
                e_single_r2 = random.choice(pos_scope_r2)
            
            e_single_r3 = random.choice(pos_scope_r3)
            while e_single_r3 in self.ignore_id:
                e_single_r3 = random.choice(pos_scope_r3)
            
            all_pos_pair.append([('2', e1, e2, e1_2, e2_2, sent_idx), ('1', e_single_r1, e_single_r2, e_single_r3)])
        return all_pos_pair
    ...
    def __pos_pair_R_q__(self, scope, relation2sen_soft_q, ent2sen_tok, max_sample_num):
        ...
        for i, item_1 in enumerate(relation2sen_soft_q):
            ...
            e1 = self.get_sen(random.choice(ent2sen_tok[item_1[0]]), h_flag, hop_flag_1, 1, 1)
            e2 = self.get_sen(random.choice(ent2sen_tok[item_1[1]]), hop_flag_1, hop_flag_2, 2, 2)
            e3 = self.get_sen(random.choice(ent2sen_tok[item_1[2]]), hop_flag_2, t_flag, 4, 3)
    ...
    def __getitem__(self, index):
        ...
        for i, ind in enumerate(bag):
            if ind[0] == '1':
                ...
                mask[i*self.args.max_length : (i+1)*self.args.max_length] = self.mask[ind]
                label[i] = self.label[ind]
                h_pos[i], t_pos[i], h_pos_l[i], t_pos_l[i] = self.h_pos[ind], self.t_pos[ind], self.h_pos_l[ind], self.t_pos_l[ind]
            else:
                assert False
        ...
        '''
        #doc sentence
        bag = self.pos_pair_doc[index]
        input_doc = np.zeros((self.args.max_length * 4), dtype=int)
        mask_doc = np.zeros((self.args.max_length * 4), dtype=int)

        input_doc_pos = np.zeros((self.args.max_length * 3), dtype=int)
        mask_doc_pos = np.zeros((self.args.max_length * 3), dtype=int)
        ...
        for i, ind in enumerate(bag):
            if ind[0] == '1':
                ...
                ind_r2 = ind[2]
                ind_r3 = ind[3]
                input_doc_pos[: self.args.max_length] = self.tokens[ind_r1]
                input_doc_pos[self.args.max_length: 2 * self.args.max_length] = self.tokens[ind_r2]
                input_doc_pos[2 * self.args.max_length: 3 * self.args.max_length] = self.tokens[ind_r3]

                mask_doc_pos[: self.args.max_length] = self.mask[ind_r1]
                mask_doc_pos[self.args.max_length: 2 * self.args.max_length] = self.mask[ind_r2]
                mask_doc_pos[2 * self.args.max_length: 3 * self.args.max_length] = self.mask[ind_r3]

                label_doc_pos = np.array([self.label[x] for x in [ind_r1, ind_r2, ind_r3]], dtype=int)
                h_doc_pos = np.array([self.h_pos[x] for x in [ind_r1, ind_r2, ind_r3]], dtype=int)
                t_doc_pos = np.array([self.t_pos[x] for x in [ind_r1, ind_r2, ind_r3]], dtype=int)
                h_l_doc_pos = np.array([self.h_pos_l[x] for x in [ind_r1, ind_r2, ind_r3]], dtype=int)
                t_l_doc_pos = np.array([self.t_pos_l[x] for x in [ind_r1, ind_r2, ind_r3]], dtype=int)
            elif ind[0] == '2':
                ids_1, ph_1, pt_1, ph_l_1, pt_l_1, r_1 = ind[1]
                ids_2, ph_2, pt_2, ph_l_2, pt_l_2, r_2 = ind[2]
                if ind[3] != None:
                    ids_1_d, ph_1_d, pt_1_d, ph_l_1_d, pt_l_1_d, r_1_d = ind[3]
                else:
                    ids_1_d, ph_1_d, pt_1_d, ph_l_1_d, pt_l_1_d, r_1_d = [], 0, 0, 0, 0, -1
                if ind[4] != None:
                    ids_2_d, ph_2_d, pt_2_d, ph_l_2_d, pt_l_2_d, r_2_d = ind[4]
                else:
                    ids_2_d, ph_2_d, pt_2_d, ph_l_2_d, pt_l_2_d, r_2_d = [], 0, 0, 0, 0, -1
                sent_idx = ind[5]
                ...
                ids_list_len = [0, len(ids_list[0]), len(ids_list[0] + ids_list[1]), len(ids_list[0] + ids_list[1] + ids_list[2])]
                if ind[3] != None:
                    ph_r1 = [ids_list_len[sent_idx[0]] + ph_1, ids_list_len[sent_idx[2]] + ph_1_d]
                    pt_r1 = [ids_list_len[sent_idx[0]] + pt_1, ids_list_len[sent_idx[2]] + pt_1_d]
                    ph_l_r1 = [ids_list_len[sent_idx[0]] + ph_l_1, ids_list_len[sent_idx[2]] + ph_l_1_d]
                    pt_l_r1 = [ids_list_len[sent_idx[0]] + pt_l_1, ids_list_len[sent_idx[2]] + pt_l_1_d]
                else:
                    ph_r1 = [ids_list_len[sent_idx[0]] + ph_1, ids_list_len[sent_idx[0]] + ph_1]
                    pt_r1 = [ids_list_len[sent_idx[0]] + pt_1, ids_list_len[sent_idx[0]] + pt_1]
                    ph_l_r1 = [ids_list_len[sent_idx[0]] + ph_l_1, ids_list_len[sent_idx[0]] + ph_l_1]
                    pt_l_r1 = [ids_list_len[sent_idx[0]] + pt_l_1, ids_list_len[sent_idx[0]] + pt_l_1]
                ph_r1 = [min(self.args.max_length*4-1, x) for x in ph_r1]
                pt_r1 = [min(self.args.max_length*4-1, x) for x in pt_r1]
                ph_l_r1 = [min(self.args.max_length*4, x) for x in ph_l_r1]
                pt_l_r1 = [min(self.args.max_length*4, x) for x in pt_l_r1]

                if ind[4] != None:
                    ph_r2 = [ids_list_len[sent_idx[1]] + ph_2, ids_list_len[sent_idx[3]] + ph_2_d]
                    pt_r2 = [ids_list_len[sent_idx[1]] + pt_2, ids_list_len[sent_idx[3]] + pt_2_d]
                    ph_l_r2 = [ids_list_len[sent_idx[1]] + ph_l_2, ids_list_len[sent_idx[3]] + ph_l_2_d]
                    pt_l_r2 = [ids_list_len[sent_idx[1]] + pt_l_2, ids_list_len[sent_idx[3]] + pt_l_2_d]
                else:
                    ph_r2 = [ids_list_len[sent_idx[1]] + ph_2, ids_list_len[sent_idx[1]] + ph_2]
                    pt_r2 = [ids_list_len[sent_idx[1]] + pt_2, ids_list_len[sent_idx[1]] + pt_2]
                    ph_l_r2 = [ids_list_len[sent_idx[1]] + ph_l_2, ids_list_len[sent_idx[1]] + ph_l_2]
                    pt_l_r2 = [ids_list_len[sent_idx[1]] + pt_l_2, ids_list_len[sent_idx[1]] + pt_l_2]
                ph_r2 = [min(self.args.max_length*4-1, x) for x in ph_r2]
                pt_r2 = [min(self.args.max_length*4-1, x) for x in pt_r2]
                ph_l_r2 = [min(self.args.max_length*4, x) for x in ph_l_r2]
                pt_l_r2 = [min(self.args.max_length*4, x) for x in pt_l_r2]
                ...
                input_doc[: length] = ids[: length]
                mask_doc[: length] = 1

                label_doc = np.array([self.rel2id[r_1], self.rel2id[r_2], self.rel2id[self.relation_map_t['#'.join([r_1, r_2])]]], dtype=int)
                h_doc = np.array(ph_r1 + ph_r2 + ph_r3, dtype=int)
                t_doc = np.array(pt_r1 + pt_r2 + pt_r3, dtype=int)
                h_l_doc = np.array(ph_l_r1 + ph_l_r2 + ph_l_r3, dtype=int)
                t_l_doc = np.array(pt_l_r1 + pt_l_r2 + pt_l_r3, dtype=int)
                ...
                # try:
                #     print([r2n[r_1], r2n[r_2], r2n[self.relation_map_t['#'.join([r_1, r_2])]]])
                #     s_y = [self.idx2token[x] for x in input_doc if x != 0]
                #     print(' '.join(s_y))
                #     for i in range(6):
                #         print(s_y[h_doc[i]: h_l_doc[i]])
                #         print(s_y[t_doc[i]: t_l_doc[i]])
                #     print('\n')
                # except:
                #     print('wrong here')

        #negative sampling for doc
        index_neg = random.randint(0, len(self.pos_pair) - 1)
        while index_neg in self.ignore_id:
            index_neg = random.randint(0, len(self.pos_pair) - 1)
        
        bag = self.pos_pair[index_neg]
        input_doc_neg = np.zeros((self.args.max_length), dtype=int)
        mask_doc_neg = np.zeros((self.args.max_length), dtype=int)
        label_doc_neg = np.zeros((1), dtype=int)
        h_doc_neg = np.zeros((1), dtype=int)
        t_doc_neg = np.zeros((1), dtype=int)
        h_l_doc_neg = np.zeros((1), dtype=int)
        t_l_doc_neg = np.zeros((1), dtype=int)

        ind = bag[0]
        if ind[0] == '1':
            ind = ind[1]
            input_doc_neg[: self.args.max_length] = self.tokens[ind]
            mask_doc_neg[: self.args.max_length] = self.mask[ind]
            label_doc_neg[0] = self.label[ind]
            h_doc_neg[0] = self.h_pos[ind]
            t_doc_neg[0] = self.t_pos[ind]
            h_l_doc_neg[0] = self.h_pos_l[ind]
            t_l_doc_neg[0] = self.t_pos_l[ind]
        else:
            assert False
        ...
        doc = (input_doc, mask_doc, label_doc, h_doc, t_doc, h_l_doc, t_l_doc)
        doc_pos = (input_doc_pos, mask_doc_pos, label_doc_pos, h_doc_pos, t_doc_pos, h_l_doc_pos, t_l_doc_pos)
        doc_neg = (input_doc_neg, mask_doc_neg, label_doc_neg, h_doc_neg, t_doc_neg, h_l_doc_neg, t_l_doc_neg)
        return single_sentence, two_hop_pos, three_hop_pos, neg, doc, doc_pos, doc_neg
