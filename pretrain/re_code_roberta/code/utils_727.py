...
class EntityMarker():
    ...
    def tokenize(self, raw_text, h_pos_li, t_pos_li, h_type=None, t_type=None, h_blank=False, t_blank=False, hop = 0):
        tokens = []
        h_mention = [] 
        t_mention = []
        ...
        if h_blank:
            if hop == 2:
                p_text = self.h_pattern.sub("[unused0] [unused6] [unused1]", p_text)
            else:
        ...
        if t_blank:
            if hop == 1:
                p_text = self.t_pattern.sub("[unused2] [unused6] [unused3]", p_text)
            else:
